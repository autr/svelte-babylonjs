			...
			<script>
				...
				*/
				...
				export let planeNormal = undefined // [vector3] * required
				...
				export let tessellation = undefined // [number] 
				...
				export let useEulerRotation = undefined // [boolean] 
				...
				export let instance = browser ? new BB.PlaneRotationGizmo(planeNormal,color,gizmoLayer,tessellation,parent,useEulerRotation,thickness) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:PlaneRotationGizmo]`, ...args) : null
					...
					const unsubscribe = bbIniting.subscribe( v => {
						...
						timeout = setTimeout( e => {
							...
							if (!$bbIniting) {
												...
												if (planeNormal != undefined) instance.planeNormal = planeNormal?.instance || planeNormal
				planeNormal = instance.planeNormal
				...
				if (tessellation != undefined) instance.tessellation = tessellation?.instance || tessellation
				tessellation = instance.tessellation
				if (parent != undefined) instance.parent = parent?.instance || parent
				parent = instance.parent
				if (useEulerRotation != undefined) instance.useEulerRotation = useEulerRotation?.instance || useEulerRotation
				useEulerRotation = instance.useEulerRotation
