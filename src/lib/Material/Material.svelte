			...
			<script>
				...
				*/
				...
				export let doNotAdd = undefined // [boolean] 
				// specifies if the material should be added to the scene
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.Material(name,scene,doNotAdd) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:Material]`, ...args) : null
				...
				if (doNotAdd != undefined) instance.doNotAdd = doNotAdd?.instance || doNotAdd
				doNotAdd = instance.doNotAdd
