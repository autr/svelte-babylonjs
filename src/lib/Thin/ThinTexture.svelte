			...
			<script>
				...
				*/
				...
				export let internalTexture = undefined // [nullable] * required
				// define the internaltexture to wrap
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.ThinTexture(internalTexture) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:ThinTexture]`, ...args) : null
					...
					const unsubscribe = bbIniting.subscribe( v => {
						...
						timeout = setTimeout( e => {
							...
							if (!$bbIniting) {
												...
												if (internalTexture != undefined) instance.internalTexture = internalTexture?.instance || internalTexture
				internalTexture = instance.internalTexture
