			...
			<script>
				...
				*/
				...
				export let colorTableUrl = undefined // [string] * required
				// 
				...
				export let instance = browser ? new BB.ColorCorrectionPostProcess(name,colorTableUrl,options,camera,samplingMode,engine,reusable) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:ColorCorrectionPostProcess]`, ...args) : null
				...
				name = instance.name
				if (colorTableUrl != undefined) instance.colorTableUrl = colorTableUrl?.instance || colorTableUrl
				colorTableUrl = instance.colorTableUrl
