			...
			<script>
				...
				*/
				...
				export let compression = undefined // [nullable] 
				// defines the compression used (null by default)
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.RawCubeTexture(scene,data,size,format,type,generateMipMaps,invertY,samplingMode,compression) : {}
				...
				export let forcedExtension = undefined // [nullable] 
				// 
				...
				export let updateRGBDAsync = (...args) => instance.updateRGBDAsync(...args)
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:RawCubeTexture]`, ...args) : null
				...
				if (compression != undefined) instance.compression = compression?.instance || compression
