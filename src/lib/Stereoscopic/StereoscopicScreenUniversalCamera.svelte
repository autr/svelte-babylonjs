			...
			<script>
				...
				*/
				...
				export let distanceToProjectionPlane = undefined // [number] 
				// defines distance between each color axis. the rig cameras will receive this as their negative z position!
				

				export let distanceBetweenEyes = undefined // [number] 
				// defines is stereoscopic is done side by side or over under
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.StereoscopicScreenUniversalCamera(name,position,scene,distanceToProjectionPlane,distanceBetweenEyes) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:StereoscopicScreenUniversalCamera]`, ...args) : null
				...
				if (distanceToProjectionPlane != undefined) instance.distanceToProjectionPlane = distanceToProjectionPlane?.instance || distanceToProjectionPlane
				distanceToProjectionPlane = instance.distanceToProjectionPlane
				if (distanceBetweenEyes != undefined) instance.distanceBetweenEyes = distanceBetweenEyes?.instance || distanceBetweenEyes
				distanceBetweenEyes = instance.distanceBetweenEyes
