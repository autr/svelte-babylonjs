			...
			<script>
				...
				*/
				...
				// defines postprocess name
				

				export let options = undefined // [union] * required
				// defines creation options or target ratio scale
				

				export let rigCameras = undefined // [array] * required
				// defines cameras using this postprocess
				

				export let samplingMode = undefined // [number] 
				// defines required sampling mode (babylon.texture.nearest_samplingmode by default)
				

				export let engine = undefined // [engine] 
				// defines hosting engine
				

				export let reusable = undefined // [boolean] 
				// defines if the postprocess will be reused multiple times per frame
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.AnaglyphPostProcess(name,options,rigCameras,samplingMode,engine,reusable) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:AnaglyphPostProcess]`, ...args) : null
