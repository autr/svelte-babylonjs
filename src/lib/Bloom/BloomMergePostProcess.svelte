			...
			<script>
				...
				*/
				...
				export let blurred = undefined // [postprocess] * required
				// blurred highlights post process which's output will be used.
				

				export let weight = undefined // [number] * required
				...
				export let instance = browser ? new BB.BloomMergePostProcess(name,originalFromInput,blurred,weight,options,camera,samplingMode,engine,reusable,textureType,blockCompilation) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:BloomMergePostProcess]`, ...args) : null
				...
				if (blurred != undefined) instance.blurred = blurred?.instance || blurred
