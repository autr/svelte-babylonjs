			...
			<script>
				...
				*/
				...
				export let pointA = undefined // [vector3] * required
				// starting point that defines the capsule segment
				

				export let pointB = undefined // [vector3] * required
				...
				export let instance = browser ? new BB.PhysicsShapeCapsule(pointA,pointB,radius,scene) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:PhysicsShapeCapsule]`, ...args) : null
					...
					const unsubscribe = bbIniting.subscribe( v => {
						...
						timeout = setTimeout( e => {
							...
							if (!$bbIniting) {
												...
												if (pointA != undefined) instance.pointA = pointA?.instance || pointA
				pointA = instance.pointA
				if (pointB != undefined) instance.pointB = pointB?.instance || pointB
				pointB = instance.pointB
