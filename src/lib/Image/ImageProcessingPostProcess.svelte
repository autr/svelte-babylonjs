			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.ImageProcessingPostProcess(name,options,camera,samplingMode,engine,reusable,textureType,imageProcessingConfiguration) : {}
				...
				export let fromLinearSpace = undefined // [boolean] 
				// 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:ImageProcessingPostProcess]`, ...args) : null
				...
				if (fromLinearSpace != undefined) instance.fromLinearSpace = fromLinearSpace?.instance || fromLinearSpace
				fromLinearSpace = instance.fromLinearSpace
