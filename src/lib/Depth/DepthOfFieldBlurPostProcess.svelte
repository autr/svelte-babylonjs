			...
			<script>
				...
				*/
				...
				export let direction = undefined // [vector2] * required
				// the direction the blur should be applied.
				

				export let kernel = undefined // [number] * required
				...
				export let circleOfConfusion = undefined // [postprocess] * required
				// the circle of confusion + depth map to be used to avoid blurring across edges
				

				export let imageToBlur = undefined // [nullable] 
				...
				export let instance = browser ? new BB.DepthOfFieldBlurPostProcess(name,scene,direction,kernel,options,camera,circleOfConfusion,imageToBlur,samplingMode,engine,reusable,textureType,blockCompilation,textureFormat) : {}
				...
				export let packedFloat = undefined // [boolean] 
				// 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:DepthOfFieldBlurPostProcess]`, ...args) : null
				...
				if (circleOfConfusion != undefined) instance.circleOfConfusion = circleOfConfusion?.instance || circleOfConfusion
				circleOfConfusion = instance.circleOfConfusion
				if (imageToBlur != undefined) instance.imageToBlur = imageToBlur?.instance || imageToBlur
				imageToBlur = instance.imageToBlur
				...
				if (textureFormat != undefined) instance.textureFormat = textureFormat?.instance || textureFormat
				...
				if (packedFloat != undefined) instance.packedFloat = packedFloat?.instance || packedFloat
				packedFloat = instance.packedFloat
