			...
			<script>
				...
				*/
				...
				export let originalFromInput = undefined // [postprocess] * required
				// post process which's input will be used for the merge.
				

				export let circleOfConfusion = undefined // [postprocess] * required
				// circle of confusion post process which's output will be used to blur each pixel.
				

				export let _blurSteps = undefined // [array] * required
				// blur post processes from low to high which will be mixed with the original image.
				...
				export let instance = browser ? new BB.DepthOfFieldMergePostProcess(name,originalFromInput,circleOfConfusion,_blurSteps,options,camera,samplingMode,engine,reusable,textureType,blockCompilation) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:DepthOfFieldMergePostProcess]`, ...args) : null
				...
				if (_blurSteps != undefined) instance._blurSteps = _blurSteps?.instance || _blurSteps
				_blurSteps = instance._blurSteps
