			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.PBRMetallicRoughnessMaterial(name,scene) : {}
				...
				export let baseColor = undefined // [color3] 
				// the base color has two different interpretations depending on the value of metalness.when the material is a metal, the base color is the specific measured reflectance valueat normal incidence (f0). for a non-metal the base color represents the reflected diffuse colorof the material.
				

				export let baseTexture = undefined // [nullable] 
				// base texture of the metallic workflow. it contains both the basecolor information in rgb aswell as opacity information in the alpha channel.
				...
				export let metallic = undefined // [number] 
				...
				export let metallicRoughnessTexture = undefined // [nullable] 
				// texture containing both the metallic value in the b channel and theroughness value in the g channel to keep better precision.
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:PBRMetallicRoughnessMaterial]`, ...args) : null
				...
				if (baseColor != undefined) instance.baseColor = baseColor?.instance || baseColor
				baseColor = instance.baseColor
				if (baseTexture != undefined) instance.baseTexture = baseTexture?.instance || baseTexture
				baseTexture = instance.baseTexture
				...
				if (metallicRoughnessTexture != undefined) instance.metallicRoughnessTexture = metallicRoughnessTexture?.instance || metallicRoughnessTexture
				metallicRoughnessTexture = instance.metallicRoughnessTexture
