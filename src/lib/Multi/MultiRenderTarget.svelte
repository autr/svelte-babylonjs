			...
			<script>
				...
				*/
				...
				export let size = undefined // [any] * required
				// define the size of the buffers to render to
				

				export let count = undefined // [number] * required
				// define the number of target we are rendering into
				...
				export let options = undefined // [imultirendertargetoptions] 
				// define the options used to create the multi render target
				

				export let textureNames = undefined // [array] 
				// define the names to set to the textures (if count > 0 - optional)
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.MultiRenderTarget(name,size,count,scene,options,textureNames) : {}
				...
				export let depthTexture = undefined // [texture] 
				// 
				...
				export let wrapU = undefined // [] 
				// 
				

				export let wrapV = undefined // [] 
				// 
				...
				export let releaseInternalTextures = (...args) => instance.releaseInternalTextures(...args)
				...
				export let setInternalTexture = (...args) => instance.setInternalTexture(...args)
				...
				export let updateCount = (...args) => instance.updateCount(...args)
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:MultiRenderTarget]`, ...args) : null
				...
				size = instance.size
				if (count != undefined) instance.count = count?.instance || count
				count = instance.count
				if (scene != undefined) instance.scene = scene?.instance || scene
				scene = instance.scene
				if (options != undefined) instance.options = options?.instance || options
				options = instance.options
				if (textureNames != undefined) instance.textureNames = textureNames?.instance || textureNames
				textureNames = instance.textureNames
