			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.WaveBlock(name) : {}
				...
				export let kind = undefined // [waveblockkind] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:WaveBlock]`, ...args) : null
				...
				if (kind != undefined) instance.kind = kind?.instance || kind
				kind = instance.kind
