			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.SkyMaterial(name,scene) : {}
				...
				// defines the solar azimuth in interval [0, 1]. the azimuth is the angle in the horizontal plan betweenan object direction and a reference direction.
				

				export let cameraOffset = undefined // [vector3] 
				...
				export let dithering = undefined // [boolean] 
				...
				export let inclination = undefined // [number] 
				...
				export let luminance = undefined // [number] 
				...
				export let mieCoefficient = undefined // [number] 
				// defines the miecoefficient in interval [0, 0.1] which affects the property .miedirectionalg.
				

				export let mieDirectionalG = undefined // [number] 
				...
				export let rayleigh = undefined // [number] 
				...
				export let sunPosition = undefined // [vector3] 
				// defines the sun position in the sky on (x,y,z). if the property .usesunposition is set to false, thenthe property is overridden by the inclination and the azimuth and can be read at any moment.
				

				export let turbidity = undefined // [number] 
				...
				export let useSunPosition = undefined // [boolean] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:SkyMaterial]`, ...args) : null
				...
				if (azimuth != undefined) instance.azimuth = azimuth?.instance || azimuth
				azimuth = instance.azimuth
				if (cameraOffset != undefined) instance.cameraOffset = cameraOffset?.instance || cameraOffset
				...
				if (dithering != undefined) instance.dithering = dithering?.instance || dithering
				dithering = instance.dithering
				...
				id = instance.id
				if (inclination != undefined) instance.inclination = inclination?.instance || inclination
				...
				if (luminance != undefined) instance.luminance = luminance?.instance || luminance
				luminance = instance.luminance
				if (metadata != undefined) instance.metadata = metadata?.instance || metadata
				metadata = instance.metadata
				if (mieCoefficient != undefined) instance.mieCoefficient = mieCoefficient?.instance || mieCoefficient
				mieCoefficient = instance.mieCoefficient
				if (mieDirectionalG != undefined) instance.mieDirectionalG = mieDirectionalG?.instance || mieDirectionalG
				mieDirectionalG = instance.mieDirectionalG
				...
				if (rayleigh != undefined) instance.rayleigh = rayleigh?.instance || rayleigh
				rayleigh = instance.rayleigh
				...
				if (sunPosition != undefined) instance.sunPosition = sunPosition?.instance || sunPosition
				sunPosition = instance.sunPosition
				if (turbidity != undefined) instance.turbidity = turbidity?.instance || turbidity
				turbidity = instance.turbidity
				if (uniqueId != undefined) instance.uniqueId = uniqueId?.instance || uniqueId
				uniqueId = instance.uniqueId
				if (up != undefined) instance.up = up?.instance || up
				up = instance.up
				if (useSunPosition != undefined) instance.useSunPosition = useSunPosition?.instance || useSunPosition
				useSunPosition = instance.useSunPosition
