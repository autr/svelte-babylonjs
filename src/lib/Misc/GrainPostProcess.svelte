			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.GrainPostProcess(name,options,camera,samplingMode,engine,reusable,textureType,blockCompilation) : {}
				...
				export let animated = undefined // [boolean] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:GrainPostProcess]`, ...args) : null
				...
				if (animated != undefined) instance.animated = animated?.instance || animated
