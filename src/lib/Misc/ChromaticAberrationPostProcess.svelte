			...
			<script>
				...
				/* 
					name  * required
					screenWidth  * required
					screenHeight  * required
					options  * required
					camera  * required
					samplingMode  
					engine  
				...
				*/
				...
				export let screenWidth = undefined // [number] * required
				// the width of the screen to apply the effect on.
				

				export let screenHeight = undefined // [number] * required
				// the height of the screen to apply the effect on.
				

				export let options = undefined // [union] * required
				...
				export let instance = browser ? new BB.ChromaticAberrationPostProcess(name,screenWidth,screenHeight,options,camera,samplingMode,engine,reusable,textureType,blockCompilation) : {}
				...
				export let aberrationAmount = undefined // [number] 
				...
				export let centerPosition = undefined // [vector2] 
				...
				export let radialIntensity = undefined // [number] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:ChromaticAberrationPostProcess]`, ...args) : null
				...
				name = instance.name
				if (screenWidth != undefined) instance.screenWidth = screenWidth?.instance || screenWidth
				screenWidth = instance.screenWidth
				if (screenHeight != undefined) instance.screenHeight = screenHeight?.instance || screenHeight
				...
				if (aberrationAmount != undefined) instance.aberrationAmount = aberrationAmount?.instance || aberrationAmount
				aberrationAmount = instance.aberrationAmount
				...
				if (centerPosition != undefined) instance.centerPosition = centerPosition?.instance || centerPosition
				...
				if (radialIntensity != undefined) instance.radialIntensity = radialIntensity?.instance || radialIntensity
				radialIntensity = instance.radialIntensity
