			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.SharpenPostProcess(name,options,camera,samplingMode,engine,reusable,textureType,blockCompilation) : {}
				...
				export let colorAmount = undefined // [number] 
				// how much of the original color should be applied. setting this to 0 will display edge detection. (default: 1)
				

				export let edgeAmount = undefined // [number] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:SharpenPostProcess]`, ...args) : null
				...
				if (colorAmount != undefined) instance.colorAmount = colorAmount?.instance || colorAmount
				colorAmount = instance.colorAmount
				if (edgeAmount != undefined) instance.edgeAmount = edgeAmount?.instance || edgeAmount
				edgeAmount = instance.edgeAmount
