			...
			<script>
				...
				*/
				...
				export let kernelMatrix = undefined // [matrix] * required
				...
				export let instance = browser ? new BB.FilterPostProcess(name,kernelMatrix,options,camera,samplingMode,engine,reusable) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:FilterPostProcess]`, ...args) : null
				...
				name = instance.name
				if (kernelMatrix != undefined) instance.kernelMatrix = kernelMatrix?.instance || kernelMatrix
				kernelMatrix = instance.kernelMatrix
