			...
			<script>
				...
				*/
				...
				export let generator = undefined // [transformnode] * required
				// the mesh or transform node to generate a trail.
				...
				export let diameter = undefined // [number] 
				// diameter of trailing mesh. default is 1.
				

				export let length = undefined // [number] 
				// length of trailing mesh. default is 60.
				

				export let autoStart = undefined // [boolean] 
				// automatically start trailing mesh. default true.
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.TrailMesh(name,generator,scene,diameter,length,autoStart) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:TrailMesh]`, ...args) : null
				...
				name = instance.name
				if (generator != undefined) instance.generator = generator?.instance || generator
				generator = instance.generator
				if (scene != undefined) instance.scene = scene?.instance || scene
				scene = instance.scene
				if (diameter != undefined) instance.diameter = diameter?.instance || diameter
				diameter = instance.diameter
				if (length != undefined) instance.length = length?.instance || length
				length = instance.length
				if (autoStart != undefined) instance.autoStart = autoStart?.instance || autoStart
