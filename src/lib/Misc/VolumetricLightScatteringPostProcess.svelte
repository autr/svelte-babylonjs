			...
			<script>
				...
				*/
				...
				export let camera = undefined // [nullable] * required
				...
				export let instance = browser ? new BB.VolumetricLightScatteringPostProcess(name,ratio,camera,mesh,samples,samplingMode,engine,reusable,scene) : {}
				...
				export let useDiffuseColor = undefined // [] 
				// 
				...
				export let attachedNode = undefined // [reflection] 
				...
				export let customMeshPosition = undefined // [vector3] 
				// custom position of the mesh. used if "usecustommeshposition" is set to "true"
				

				export let decay = undefined // [number] 
				...
				export let includedMeshes = undefined // [array] 
				// array containing the only meshes rendered in the internal pass.if this array is not empty, only the meshes from this array are rendered in the internal pass
				...
				export let invert = undefined // [boolean] 
				...
				export let useCustomMeshPosition = undefined // [boolean] 
				...
				export let getCustomMeshPosition = (...args) => instance.getCustomMeshPosition(...args)
				...
				export let getPass = (...args) => instance.getPass(...args)
				...
				export let setCustomMeshPosition = (...args) => instance.setCustomMeshPosition(...args)
				...
				export let CreateDefaultMesh = (...args) => instance.CreateDefaultMesh(...args)
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:VolumetricLightScatteringPostProcess]`, ...args) : null
				...
				if (useDiffuseColor != undefined) instance.useDiffuseColor = useDiffuseColor?.instance || useDiffuseColor
				useDiffuseColor = instance.useDiffuseColor
				...
				if (customMeshPosition != undefined) instance.customMeshPosition = customMeshPosition?.instance || customMeshPosition
				customMeshPosition = instance.customMeshPosition
				if (decay != undefined) instance.decay = decay?.instance || decay
				...
				if (includedMeshes != undefined) instance.includedMeshes = includedMeshes?.instance || includedMeshes
				includedMeshes = instance.includedMeshes
				...
				if (invert != undefined) instance.invert = invert?.instance || invert
				...
				if (useCustomMeshPosition != undefined) instance.useCustomMeshPosition = useCustomMeshPosition?.instance || useCustomMeshPosition
				useCustomMeshPosition = instance.useCustomMeshPosition
