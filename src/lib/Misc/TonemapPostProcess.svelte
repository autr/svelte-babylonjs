			...
			<script>
				...
				*/
				...
				// defines the name of the postprocess
				

				export let _operator = undefined // [tonemappingoperator] * required
				// defines the operator to use
				

				export let exposureAdjustment = undefined // [number] * required
				// defines the required exposure adjustment
				

				export let camera = undefined // [nullable] * required
				// defines the camera to use (can be null)
				

				export let samplingMode = undefined // [number] 
				// defines the required sampling mode (babylon.texture.bilinear_samplingmode by default)
				

				export let engine = undefined // [engine] 
				// defines the hosting engine (can be ignore if camera is set)
				

				export let textureFormat = undefined // [number] 
				// defines the texture format to use (babylon.engine.texturetype_unsigned_int by default)
				...
				export let instance = browser ? new BB.TonemapPostProcess(name,_operator,exposureAdjustment,camera,samplingMode,engine,textureFormat,reusable) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:TonemapPostProcess]`, ...args) : null
				...
				name = instance.name
				if (_operator != undefined) instance._operator = _operator?.instance || _operator
				_operator = instance._operator
				if (exposureAdjustment != undefined) instance.exposureAdjustment = exposureAdjustment?.instance || exposureAdjustment
				exposureAdjustment = instance.exposureAdjustment
