			...
			<script>
				...
				*/
				...
				export let element = undefined // [union] * required
				// defines the video or canvas the texture is filled with
				

				export let options = undefined // [ihtmlelementtextureoptions] * required
				// defines the other none mandatory texture creation options
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.HtmlElementTexture(name,element,options) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:HtmlElementTexture]`, ...args) : null
