			...
			<script>
				...
				*/
				...
				export let size = undefined // [union] 
				// the size of the render target (used for each view)
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.MultiviewRenderTarget(scene,size) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:MultiviewRenderTarget]`, ...args) : null
