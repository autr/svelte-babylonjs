			...
			<script>
				...
				*/
				...
				export let generateEdgesLines = undefined // [boolean] 
				// should generate lines or only prepare resources.
				

				export let options = undefined // [iedgesrendereroptions] 
				// the options to apply when generating the edges
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.EdgesRenderer(source,epsilon,checkVerticesInsteadOfIndices,generateEdgesLines,options) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:EdgesRenderer]`, ...args) : null
				...
				if (generateEdgesLines != undefined) instance.generateEdgesLines = generateEdgesLines?.instance || generateEdgesLines
				generateEdgesLines = instance.generateEdgesLines
