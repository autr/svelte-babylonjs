			...
			<script>
				...
				*/
				...
				export let fragmentUrl = undefined // [string] * required
				// the url of the fragment shader to be used.
				

				export let parameters = undefined // [nullable] * required
				// array of the names of uniform non-sampler2d variables that will be passed to the shader.
				

				export let samplers = undefined // [nullable] * required
				...
				export let vertexUrl = undefined // [string] 
				...
				export let instance = browser ? new BB.PostProcess(name,fragmentUrl,parameters,samplers,options,camera,samplingMode,engine,reusable,defines,textureType,vertexUrl,indexParameters,blockCompilation,textureFormat,shaderLanguage) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:PostProcess]`, ...args) : null
				...
				name = instance.name
				if (fragmentUrl != undefined) instance.fragmentUrl = fragmentUrl?.instance || fragmentUrl
				fragmentUrl = instance.fragmentUrl
				if (parameters != undefined) instance.parameters = parameters?.instance || parameters
				...
				if (vertexUrl != undefined) instance.vertexUrl = vertexUrl?.instance || vertexUrl
				vertexUrl = instance.vertexUrl
