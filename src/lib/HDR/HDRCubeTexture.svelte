			...
			<script>
				...
				*/
				...
				export let generateHarmonics = undefined // [boolean] 
				...
				export let prefilterOnLoad = undefined // [boolean] 
				...
				export let instance = browser ? new BB.HDRCubeTexture(url,sceneOrEngine,size,noMipmap,generateHarmonics,gammaSpace,prefilterOnLoad,onLoad,onError) : {}
				...
				// gets or sets the center of the bounding box associated with the cube textureit must define where the camera used to render the texture was set
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:HDRCubeTexture]`, ...args) : null
				...
				if (generateHarmonics != undefined) instance.generateHarmonics = generateHarmonics?.instance || generateHarmonics
				generateHarmonics = instance.generateHarmonics
				if (gammaSpace != undefined) instance.gammaSpace = gammaSpace?.instance || gammaSpace
				gammaSpace = instance.gammaSpace
				if (prefilterOnLoad != undefined) instance.prefilterOnLoad = prefilterOnLoad?.instance || prefilterOnLoad
				prefilterOnLoad = instance.prefilterOnLoad
