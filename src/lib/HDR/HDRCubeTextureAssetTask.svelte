			...
			<script>
				...
				*/
				...
				export let reserved = undefined // [boolean] 
				// internal use only
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.HDRCubeTextureAssetTask(name,url,size,noMipmap,generateHarmonics,gammaSpace,reserved) : {}
				...
				export let texture = undefined // [hdrcubetexture] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:HDRCubeTextureAssetTask]`, ...args) : null
				...
				if (reserved != undefined) instance.reserved = reserved?.instance || reserved
