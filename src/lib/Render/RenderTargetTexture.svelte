			...
			<script>
				...
				*/
				...
				export let options = undefined // [rendertargettextureoptions] 
				// the options for creating the render target texture.
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.RenderTargetTexture(name,size,scene,options) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:RenderTargetTexture]`, ...args) : null
