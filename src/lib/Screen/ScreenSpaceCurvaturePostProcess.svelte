			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.ScreenSpaceCurvaturePostProcess(name,scene,options,camera,samplingMode,engine,reusable,textureType,blockCompilation) : {}
				...
				export let ridge = undefined // [number] 
				...
				export let valley = undefined // [number] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:ScreenSpaceCurvaturePostProcess]`, ...args) : null
				...
				if (ridge != undefined) instance.ridge = ridge?.instance || ridge
				ridge = instance.ridge
				...
				if (valley != undefined) instance.valley = valley?.instance || valley
