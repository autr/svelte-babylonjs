			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.ScreenSpaceReflectionPostProcess(name,scene,options,camera,samplingMode,engine,reusable,textureType,blockCompilation,forceGeometryBuffer) : {}
				...
				export let reflectionSamples = undefined // [number] 
				// 
				...
				export let smoothSteps = undefined // [number] 
				// 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:ScreenSpaceReflectionPostProcess]`, ...args) : null
				...
				if (reflectionSamples != undefined) instance.reflectionSamples = reflectionSamples?.instance || reflectionSamples
				reflectionSamples = instance.reflectionSamples
				if (samples != undefined) instance.samples = samples?.instance || samples
				samples = instance.samples
				if (smoothSteps != undefined) instance.smoothSteps = smoothSteps?.instance || smoothSteps
				smoothSteps = instance.smoothSteps
