			...
			<script>
				...
				*/
				...
				export let isRightEye = undefined // [boolean] * required
				// if this is for the right eye distortion
				

				export let vrMetrics = undefined // [vrcamerametrics] * required
				// all the required metrics for the vr camera
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.VRDistortionCorrectionPostProcess(name,camera,isRightEye,vrMetrics) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:VRDistortionCorrectionPostProcess]`, ...args) : null
				...
				if (isRightEye != undefined) instance.isRightEye = isRightEye?.instance || isRightEye
				isRightEye = instance.isRightEye
				if (vrMetrics != undefined) instance.vrMetrics = vrMetrics?.instance || vrMetrics
				vrMetrics = instance.vrMetrics
