			...
			<script>
				...
				*/
				...
				export let scaleFactor = undefined // [number] * required
				// scaling factor to the size of the output texture
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.VRMultiviewToSingleviewPostProcess(name,camera,scaleFactor) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:VRMultiviewToSingleviewPostProcess]`, ...args) : null
				...
				if (scaleFactor != undefined) instance.scaleFactor = scaleFactor?.instance || scaleFactor
