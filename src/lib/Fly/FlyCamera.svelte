			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.FlyCamera(name,position,scene,setActiveOnSceneIfNoneActive) : {}
				...
				export let _trackRoll = undefined // [number] 
				...
				export let inputs = undefined // [flycamerainputsmanager] 
				...
				export let rollCorrect = undefined // [number] 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:FlyCamera]`, ...args) : null
				...
				if (_trackRoll != undefined) instance._trackRoll = _trackRoll?.instance || _trackRoll
				_trackRoll = instance._trackRoll
				...
				if (bankedTurn != undefined) instance.bankedTurn = bankedTurn?.instance || bankedTurn
				bankedTurn = instance.bankedTurn
				if (bankedTurnLimit != undefined) instance.bankedTurnLimit = bankedTurnLimit?.instance || bankedTurnLimit
				bankedTurnLimit = instance.bankedTurnLimit
				if (bankedTurnMultiplier != undefined) instance.bankedTurnMultiplier = bankedTurnMultiplier?.instance || bankedTurnMultiplier
				bankedTurnMultiplier = instance.bankedTurnMultiplier
				...
				if (rollCorrect != undefined) instance.rollCorrect = rollCorrect?.instance || rollCorrect
				rollCorrect = instance.rollCorrect
