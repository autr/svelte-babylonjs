			...
			<script>
				...
				*/
				...
				export let cellSize = undefined // [any] * required
				...
				export let fromPacked = undefined // [boolean] 
				// set to false; do not alter
				

				export let spriteJSON = undefined // [any] 
				// null otherwise a json object defining sprite sheet data; do not alter
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.SpriteManager(name,imgUrl,capacity,cellSize,scene,epsilon,samplingMode,fromPacked,spriteJSON) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:SpriteManager]`, ...args) : null
				...
				if (cellSize != undefined) instance.cellSize = cellSize?.instance || cellSize
				...
				if (fromPacked != undefined) instance.fromPacked = fromPacked?.instance || fromPacked
				fromPacked = instance.fromPacked
