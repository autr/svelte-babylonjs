			...
			<script>
				...
				*/
				...
				export let refractionTextureUrl = undefined // [string] * required
				// url of the refraction texture to use
				...
				export let colorLevel = undefined // [number] * required
				...
				export let instance = browser ? new BB.RefractionPostProcess(name,refractionTextureUrl,color,depth,colorLevel,options,camera,samplingMode,engine,reusable) : {}
				...
				export let refractionTexture = undefined // [texture] 
				// 
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:RefractionPostProcess]`, ...args) : null
				...
				name = instance.name
				if (refractionTextureUrl != undefined) instance.refractionTextureUrl = refractionTextureUrl?.instance || refractionTextureUrl
				refractionTextureUrl = instance.refractionTextureUrl
				if (color != undefined) instance.color = color?.instance || color
				color = instance.color
				if (depth != undefined) instance.depth = depth?.instance || depth
				depth = instance.depth
				if (colorLevel != undefined) instance.colorLevel = colorLevel?.instance || colorLevel
				colorLevel = instance.colorLevel
