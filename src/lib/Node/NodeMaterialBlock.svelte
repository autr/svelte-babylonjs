			...
			<script>
				...
				*/
				...
				export let instance = browser ? new BB.NodeMaterialBlock(name,target,isFinalMerger,isInput) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:NodeMaterialBlock]`, ...args) : null
				...
				if (isFinalMerger != undefined) instance.isFinalMerger = isFinalMerger?.instance || isFinalMerger
				isFinalMerger = instance.isFinalMerger
				if (isInput != undefined) instance.isInput = isInput?.instance || isInput
