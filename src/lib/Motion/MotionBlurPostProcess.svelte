			...
			<script>
				...
				*/
				...
				// the scene containing the objects to blur according to their velocity.
				...
				// if compilation of the shader should not be done in the constructor. the updateeffect method can be used to compile the shader at a later time. (default: true)
				...
				export let instance = browser ? new BB.MotionBlurPostProcess(name,scene,options,camera,samplingMode,engine,reusable,textureType,blockCompilation,forceGeometryBuffer) : {}
				...
				export let isObjectBased = undefined // [boolean] 
				// 
				...
				export let excludeSkinnedMesh = (...args) => instance.excludeSkinnedMesh(...args)
				...
				export let removeExcludedSkinnedMesh = (...args) => instance.removeExcludedSkinnedMesh(...args)
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:MotionBlurPostProcess]`, ...args) : null
				...
				if (isObjectBased != undefined) instance.isObjectBased = isObjectBased?.instance || isObjectBased
				isObjectBased = instance.isObjectBased
