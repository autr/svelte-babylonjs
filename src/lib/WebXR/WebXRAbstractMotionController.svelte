			...
			<script>
				...
				*/
				...
				export let layout = undefined // [imotioncontrollerlayout] * required
				...
				export let _controllerCache = undefined // [array] 
				// a cache holding controller models already loaded in this session
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.WebXRAbstractMotionController(scene,layout,gamepadObject,handedness,_controllerCache) : {}
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:WebXRAbstractMotionController]`, ...args) : null
				...
				if (layout != undefined) instance.layout = layout?.instance || layout
				...
				if (_controllerCache != undefined) instance._controllerCache = _controllerCache?.instance || _controllerCache
				_controllerCache = instance._controllerCache
