			...
			<script>
				...
				*/
				...
				export let name = undefined // [nullable] * required
				// optional name, will detect from video source, if not defined
				

				export let src = undefined // [union] * required
				...
				export let settings = undefined // [partial] 
				...
				// defines the texture format to use (engine.textureformat_rgba by default)
				

				// ================ CLASS INSTANCE ================

				export let instance = browser ? new BB.VideoTexture(name,src,scene,generateMipMaps,invertY,samplingMode,settings,onError,format) : {}
				...
				export let onUserActionRequestedObservable = undefined // [observable] 
				// 
				...
				export let autoUpdateTexture = undefined // [boolean] 
				// tells whether textures will be updated automatically or user is required to call 
				...
				export let video = undefined // [htmlvideoelement] 
				...
				export let updateTexture = (...args) => instance.updateTexture(...args)
				...
				export let CreateFromStreamAsync = (...args) => instance.CreateFromStreamAsync(...args)

				export let CreateFromWebCam = (...args) => instance.CreateFromWebCam(...args)

				export let CreateFromWebCamAsync = (...args) => instance.CreateFromWebCamAsync(...args)
				...
				const DEBUG = (...args) => $bbDebug ? console.log(`[${name}:VideoTexture]`, ...args) : null
				...
				name = instance.name
				if (src != undefined) instance.src = src?.instance || src
				src = instance.src
				...
				if (settings != undefined) instance.settings = settings?.instance || settings
				...
				onUserActionRequestedObservable = instance.onUserActionRequestedObservable
